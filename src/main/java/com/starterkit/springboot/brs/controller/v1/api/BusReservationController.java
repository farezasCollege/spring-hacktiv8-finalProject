...
public class BusReservationController {
            ...
            if (tripDto.isPresent()) {
                ...
                if (tripScheduleDto.isPresent()) {
                    Optional<TicketDto> ticketDto = Optional
                            .ofNullable(busReservationService.bookTicket(tripScheduleDto.get(), userDto.get(), bookTicketRequest.getFoodPackageId()));
    ...
    @GetMapping("/getfoodsbytripschedule")
    @ApiOperation(value = "", authorizations = {@Authorization(value = "apiKey")})
    public Response getFoodPackageByTripSchedule(@Valid Long TripScheduleId) {
        Optional<List<FoodBevDto>> fandbevlist = Optional.ofNullable(busReservationService.getFoodBevListByTripSchedule(TripScheduleId));

        if (fandbevlist.isPresent()) {
            return Response.ok().setPayload(fandbevlist.get());
        }
